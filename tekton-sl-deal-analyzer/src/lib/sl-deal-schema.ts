...
export const slDealInputSchema = z.object({
  ...
  interest_rate: z.number().min(0).max(0.25).default(0.1),
  cost_of_capital_rate: z.number().min(0).max(0.3).default(0.16),
  ltc_ratio: z.number().min(0).max(1).default(0.85),

  // Sales
  asset_sales_price: z.number().min(1, "ASP is required"),
  selling_cost_rate: z.number().min(0).max(0.2).default(0.085),
