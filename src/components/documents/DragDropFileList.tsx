        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-background">
              ...
              <th className="w-8 px-3 py-3" />
              <th className="text-left px-3 py-3 text-xs font-semibold text-muted uppercase tracking-wider">Name</th>
              <th className="text-left px-3 py-3 text-xs font-semibold text-muted uppercase tracking-wider">Size</th>
  ...
  if (documents.length === 0) {
    return (
      <div className="bg-white border border-border rounded-lg p-12 text-center">
        ...
        <h3 className="text-sm font-semibold text-text-secondary mb-1">No documents</h3>
        <p className="text-sm text-muted">Upload files or drag documents here to get started.</p>
      </div>
    );
  }
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-border bg-background">
            ...
            <th className="text-left px-3 py-3 text-xs font-semibold text-muted uppercase tracking-wider">Name</th>
            <th className="text-left px-3 py-3 text-xs font-semibold text-muted uppercase tracking-wider">Size</th>
            <th className="text-left px-3 py-3 text-xs font-semibold text-muted uppercase tracking-wider">Modified</th>
            <th className="w-10 px-3 py-3" />
          </tr>
        </thead>
        <tbody>
            ...
            return (
              <tr
                ...
                className={cn(
                  ...
                  isSelected ? "bg-emerald-50" : "hover:bg-background",
              ...
              >
                ...
                {/* Size */}
                <td className="px-3 py-3 text-sm text-muted">{formatFileSize(doc.file_size)}</td>

                {/* Modified */}
                <td className="px-3 py-3 text-sm text-muted">{formatDate(doc.updated_at)}</td>
                ...
                <td className="px-3 py-3 relative" onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    <span className="text-sm text-muted font-bold leading-none">&middot;&middot;&middot;</span>
                  </button>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-1 z-20 bg-white border border-border rounded-lg shadow-lg py-1 min-w-[160px]">
                        {onDownload && (
                          <button
                            type="button"
                            className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-secondary hover:bg-background"
                            onClick={() => {
                              onDownload(doc);
                        ...
                        {onDelete && (
                          <button
                            type="button"
                            className="w-full flex items-center gap-2 px-3 py-2 text-sm text-destructive hover:bg-destructive-bg"
                            onClick={() => {
                              onDelete(doc);
                              setActiveMenuId(null);
                            }}
