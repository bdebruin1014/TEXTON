...
export const Route = createFileRoute("/_authenticated/disposition/$dispositionId/warranty")({
  component: Warranty,
});
...
function Warranty() {
  ...
  const { data: claims = [], isLoading } = useQuery<WarrantyClaim[]>({
    queryKey: ["dispo-warranty-claims", dispositionId],
    queryFn: async () => {
  ...
  const addClaim = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dispo-warranty-claims", dispositionId] }),
  });
