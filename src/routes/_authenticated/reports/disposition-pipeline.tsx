  ...
  const { data: rawDisps = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("dispositions")
        .select("id, project_name, lot_number, buyer_name, status, contract_price, closing_date, contract_date, floor_plan, updated_at")
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-foreground">Disposition Pipeline</h1>
        ...
        <button
          type="button"
          onClick={() =>
            exportToCsv("disposition-pipeline", [
